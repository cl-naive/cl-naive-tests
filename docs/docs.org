...
Testcases can be skipped or disabled, so these counters may be nonzero.
See DISABLED in TESTCASE and SKIP-TESTCASE/DISABLE-TESTCASE.
...
| =:hostname=   | host running the testsuite (machine-instance)       |        string |
...
| =:reason=       | Skip reason (when failure-type is =:SKIPPED=).                                                     |        string |
| =:result=       | Same as =:FAILURE-TYPE`=                                                                           |       keyword |
| =:failure-type= | A keyword indicating the result: =:SUCCESS= =:FAILURE= =:ERROR= =:SKIPPED= =:DISABLED=.             |       keyword |
...
Signals a DISABLE-TESTCASE condition, marking the current testcase as
disabled without running it.

** [function] =skip-testcase= =(&optional (message "Test case is skipped."))=

Signals a SKIP-TESTCASE condition with a reason message. The reason is
stored in the testcase result under =:REASON=.

** [function] =skip-testcase-reason= =(condition)=

Returns the reason stored in a SKIP-TESTCASE condition.
